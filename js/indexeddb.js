...
varId: 32
      ...
      var putRequest = objectStore.put(data); // The put() method will update an existing record, whereas the add() method won't.
...
// var url = window.URL.createObjectURL(new Blob(curser.value, {'type': curser.value.type}));
   
   // link.trigger('click');

	      var videoFile = event.target.result;
        var URL = window.URL || window.webkitURL;
        var videoURL = URL.createObjectURL(cursor.value);
